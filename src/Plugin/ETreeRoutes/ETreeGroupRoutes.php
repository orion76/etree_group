<?php


namespace Drupal\etree_group\Plugin\ETreeRoutes;


use Drupal;
use Drupal\Component\Plugin\Context\ContextInterface;
use Drupal\Core\Config\Entity\ConfigEntityTypeInterface;
use Drupal\Core\Entity\Controller\EntityController;
use Drupal\Core\Entity\EntityTypeInterface;
use Drupal\Core\Entity\FieldableEntityInterface;
use Drupal\etree\Entity\ETree;
use Drupal\etree\Plugin\ETreeRoutesPluginBase;
use Drupal\etree\Plugin\ETreeRoutesPluginInterface;
use Drupal\etree_context\Context\ETreeRouteContextInterface;
use Symfony\Component\DependencyInjection\ContainerInterface;
use Symfony\Component\Routing\Route;

/**
 * Class ETreeGroupRoutes
 *
 * @ETreeRoutes(
 *   id = "etree_group",
 *   label = @Translation("ETree Group")
 * )
 *
 * @package Drupal\etree_group
 */
class ETreeGroupRoutes extends ETreeRoutesPluginBase {

  use Drupal\Core\Messenger\MessengerTrait;


  /**
   * @var \Drupal\Core\Entity\EntityStorageBase
   */
  protected $groupStorage;

  protected $entityFieldManager;

  protected $entityTypedManager;

  /** @var \Drupal\etree_group\EtreeGroupHelper $groupHelper */
  protected $groupHelper;

  /**
   * @var \Drupal\etree_context\Context\ETreeRouteContextInterface
   */
  private $context;

  public function __construct(array $configuration,
                              $plugin_id,
                              $plugin_definition,
                              ETreeRouteContextInterface $context) {
    parent::__construct($configuration, $plugin_id, $plugin_definition);

    $this->context = $context;
  }

  /**
   * {@inheritdoc}
   */
  public static function create(ContainerInterface $container, array $configuration, $plugin_id, $plugin_definition) {
    $n = 0;
    return new static(
      $configuration,
      $plugin_id,
      $plugin_definition,
      $container->get('etree_context.context_route')
    );
  }

  /**
   * @return \Drupal\Core\Entity\EntityFieldManagerInterface
   */
  protected function getEntityFieldManager() {
    if (!$this->entityFieldManager) {
      $this->entityFieldManager = Drupal::service('entity_field.manager');
    }
    return $this->entityFieldManager;
  }

  protected function getGroupHelper() {
    if (!$this->groupHelper) {
      $this->groupHelper = Drupal::service('etree_group.helper');
    }
    return $this->groupHelper;
  }


  /**
   * @return \Drupal\Core\Entity\EntityStorageBase|\Drupal\Core\Entity\EntityStorageInterface
   */
  protected function getGroupStorage() {
    if (is_null($this->groupStorage)) {
      try {
        $this->groupStorage = $this->getEntityTypeManager()->getStorage('etree_group');
      } catch (Drupal\Component\Plugin\Exception\InvalidPluginDefinitionException $e) {
        $this->messenger()->addError($e);
      } catch (Drupal\Component\Plugin\Exception\PluginNotFoundException $e) {
        $this->messenger()->addError($e);
      }
    }
    return $this->groupStorage;
  }

  /**
   * @return \Drupal\Core\Entity\EntityTypeManagerInterface
   */
  protected function getEntityTypeManager() {
    if (is_null($this->entityTypedManager)) {
      $this->entityTypedManager = Drupal::service('entity_type.manager');
    }
    return $this->entityTypedManager;
  }

  public function getRouteName($group_id, $action) {
    if ($view_name = $this->context->getRuntimeContext('etree_group_view')) {
      return $this->createRouteNameView($group_id, $view_name, $action);
    }
  }

  /**
   * {@inheritdoc}
   */
  public function getRoutes() {


    $actions_map = [
      'canonical' => [
        'callback' => '_getCanonicalRoute',
      ],
      'add-page' => [
        'callback' => '_getAddPageRoute',
      ],
      'add-form' => [
        'callback' => '_getAddFormRoute',
      ],
      'edit-form' => [
        'callback' => '_getEditFormRoute',
      ],
      'collection' => [
        'callback' => '_getCollectionRoute',
      ],
      //_getDeleteFormRoute
      'delete-form' => [
        'callback' => '_getDeleteFormRoute',
      ],
      //delete-multiple-confirm
      'delete-multiple-confirm' => [
        'callback' => '_getDeleteMultipleFormRoute',
      ],
    ];

    $entity_type = $this->getEntityTypeManager()->getDefinition('etree');
    $groups = $this->getGroupStorage()->loadMultiple();

    return $this->_getRoutes($entity_type, $groups, $actions_map);
  }


  private static function getGroupPath($group_id) {
    return "admin/structure/etree/group/$group_id";
  }

  protected function createGroupRoutes(EntityTypeInterface $entity_type, $action, $data) {
    $link_template = $entity_type->getLinkTemplate($action);
    $routes = [];
    foreach ($this->getGroupStorage()->loadMultiple() as $group_id => $group) {
      $group_link_template = str_replace(ETree::getStandardPath(), static::getGroupPath($group_id), $link_template);
      if ($route = call_user_func_array([$this, $data['callback']], [$entity_type, $group_link_template])) {
        $route_name = ETree::createRouteNameGroup($group_id, $action);
        $routes[$route_name] = $route;
      }
    }
    return $routes;

  }

  protected function _getRoutes(EntityTypeInterface $entity_type, $groups, $action_map) {
    $routes = [];
    $standard_path = ETree::getStandardPath();
    foreach ($action_map as $action => $data) {
      if ($entity_type->hasLinkTemplate($action)) {

        $routes += $this->createGroupRoutes($entity_type, $action, $data);

        $link_template = $entity_type->getLinkTemplate($action);
        foreach ($this->getGroupStorage()->loadMultiple() as $group_id => $group) {
          /** @var \Drupal\etree_group\Entity\ETreeGroupInterface $group */

//          foreach ($group->getCollectionPaths() as $view_id => $path) {
//            /** @var \Drupal\etree_group\Entity\ETreeGroupInterface $group */
//            $group_link_template = str_replace($standard_path, $path, $link_template);
//
//            if ($route = call_user_func_array([$this, $data['callback']], [$entity_type, $group_link_template])) {
//              $route_name = $this->createRouteNameView($group_id, $view_id, $action);
//              $routes[$route_name] = $route;
//            }
//
//          }
        }
      }
    }

    return $routes;
  }

  private function createRouteNameView($group_id, $view_id, $action) {
    $route_name = "entity.etree_group.{$group_id}.{$view_id}.{$action}";
    return str_replace('-', '_', $route_name);
  }

  /**
   * Gets the edit-form route.
   *
   * @param \Drupal\Core\Entity\EntityTypeInterface $entity_type
   *   The entity type.
   *
   * @return \Symfony\Component\Routing\Route|null
   *   The generated route, if available.
   */
  protected function _getEditFormRoute(EntityTypeInterface $entity_type, $group_link_template) {
    $entity_type_id = $entity_type->id();
    $route = new Route($group_link_template);
    // Use the edit form handler, if available, otherwise default.
    $operation = 'default';
    if ($entity_type->getFormClass('edit')) {
      $operation = 'edit';
    }
    $route
      ->setDefaults([
        '_entity_form' => "{$entity_type_id}.{$operation}",
        '_title_callback' => '\Drupal\Core\Entity\Controller\EntityController::editTitle',
      ])
      ->setRequirement('_entity_access', "{$entity_type_id}.update")
      ->setOption('parameters', [
        $entity_type_id => ['type' => 'entity:' . $entity_type_id],
      ]);

    // Entity types with serial IDs can specify this in their route
    // requirements, improving the matching process.
    if ($this->getEntityTypeIdKeyType($entity_type) === 'integer') {
      $route->setRequirement($entity_type_id, '\d+');
    }
    return $route;
  }

  /**
   * Gets the delete-form route.
   *
   * @param \Drupal\Core\Entity\EntityTypeInterface $entity_type
   *   The entity type.
   *
   * @return \Symfony\Component\Routing\Route|null
   *   The generated route, if available.
   */
  protected function _getDeleteFormRoute(EntityTypeInterface $entity_type, $group_link_template) {

    $entity_type_id = $entity_type->id();
    $route = new Route($group_link_template);
    $route
      ->addDefaults([
        '_entity_form' => "{$entity_type_id}.delete",
        '_title_callback' => '\Drupal\Core\Entity\Controller\EntityController::deleteTitle',
      ])
      ->setRequirement('_entity_access', "{$entity_type_id}.delete")
      ->setOption('parameters', [
        $entity_type_id => ['type' => 'entity:' . $entity_type_id],
      ]);

    // Entity types with serial IDs can specify this in their route
    // requirements, improving the matching process.
    if ($this->getEntityTypeIdKeyType($entity_type) === 'integer') {
      $route->setRequirement($entity_type_id, '\d+');
    }
    return $route;

  }

  protected function _getDeleteMultipleFormRoute(EntityTypeInterface $entity_type, $group_link_template) {
    if ($entity_type->hasHandlerClass('form', 'delete-multiple-confirm')) {
      $route = new Route($group_link_template);
      $route->setDefault('_form', $entity_type->getFormClass('delete-multiple-confirm'));
      $route->setDefault('entity_type_id', $entity_type->id());
      $route->setRequirement('_entity_delete_multiple_access', $entity_type->id());
      return $route;
    }
  }

  /**
   * Gets the canonical route.
   *
   * @param \Drupal\Core\Entity\EntityTypeInterface $entity_type
   *   The entity type.
   *
   * @return \Symfony\Component\Routing\Route|null
   *   The generated route, if available.
   */
  protected function _getCanonicalRoute(EntityTypeInterface $entity_type, $group_link_template) {
    if ($entity_type->hasViewBuilderClass()) {
      $entity_type_id = $entity_type->id();
      $route = new Route($group_link_template);
      $route
        ->addDefaults([
          '_entity_view' => "{$entity_type_id}.full",
          '_title_callback' => '\Drupal\Core\Entity\Controller\EntityController::title',
        ])
        ->setRequirement('_entity_access', "{$entity_type_id}.view")
        ->setOption('parameters', [
          $entity_type_id => ['type' => 'entity:' . $entity_type_id],
        ]);

      // Entity types with serial IDs can specify this in their route
      // requirements, improving the matching process.
      if ($this->getEntityTypeIdKeyType($entity_type) === 'integer') {
        $route->setRequirement($entity_type_id, '\d+');
      }
      return $route;
    }
  }

  /**
   * Gets the add-form route.
   *
   * @param \Drupal\Core\Entity\EntityTypeInterface $entity_type
   *   The entity type.
   *
   * @return \Symfony\Component\Routing\Route|null
   *   The generated route, if available.
   */
  protected function _getAddFormRoute(EntityTypeInterface $entity_type, $group_link_template) {

    $entity_type_id = $entity_type->id();
    $route = new Route($group_link_template);
    // Use the add form handler, if available, otherwise default.
    $operation = 'default';
    if ($entity_type->getFormClass('add')) {
      $operation = 'add';
    }
    $route->setDefaults([
      '_entity_form' => "{$entity_type_id}.{$operation}",
      'entity_type_id' => $entity_type_id,
    ]);

    // If the entity has bundles, we can provide a bundle-specific title
    // and access requirements.
    $expected_parameter = $entity_type->getBundleEntityType() ?: $entity_type->getKey('bundle');
    // @todo: We have to check if a route contains a bundle in its path as
    //   test entities have inconsistent usage of "add-form" link templates.
    //   Fix it in https://www.drupal.org/node/2699959.
    if (($bundle_key = $entity_type->getKey('bundle')) && strpos($route->getPath(), '{' . $expected_parameter . '}') !== FALSE) {
      $route->setDefault('_title_callback', EntityController::class . '::addBundleTitle');
      // If the bundles are entities themselves, we can add parameter
      // information to the route options.
      if ($bundle_entity_type_id = $entity_type->getBundleEntityType()) {
        $bundle_entity_type = $this->getEntityTypeManager()->getDefinition($bundle_entity_type_id);

        $route
          // The title callback uses the value of the bundle parameter to
          // fetch the respective bundle at runtime.
          ->setDefault('bundle_parameter', $bundle_entity_type_id)
          ->setRequirement('_entity_create_access', $entity_type_id . ':{' . $bundle_entity_type_id . '}');

        // Entity types with serial IDs can specify this in their route
        // requirements, improving the matching process.
        if ($this->getEntityTypeIdKeyType($bundle_entity_type) === 'integer') {
          $route->setRequirement($entity_type_id, '\d+');
        }

        $bundle_entity_parameter = ['type' => 'entity:' . $bundle_entity_type_id];
        if ($bundle_entity_type instanceof ConfigEntityTypeInterface) {
          // The add page might be displayed on an admin path. Even then, we
          // need to load configuration overrides so that, for example, the
          // bundle label gets translated correctly.
          // @see \Drupal\Core\ParamConverter\AdminPathConfigEntityConverter
          $bundle_entity_parameter['with_config_overrides'] = TRUE;
        }
        $route->setOption('parameters', [$bundle_entity_type_id => $bundle_entity_parameter]);
      }
      else {
        // If the bundles are not entities, the bundle key is used as the
        // route parameter name directly.
        $route
          ->setDefault('bundle_parameter', $bundle_key)
          ->setRequirement('_entity_create_access', $entity_type_id . ':{' . $bundle_key . '}');
      }
    }
    else {
      $route
        ->setDefault('_title_callback', EntityController::class . '::addTitle')
        ->setRequirement('_entity_create_access', $entity_type_id);
    }

    return $route;

  }

  /**
   * Gets the type of the ID key for a given entity type.
   *
   * @param \Drupal\Core\Entity\EntityTypeInterface $entity_type
   *   An entity type.
   *
   * @return string|null
   *   The type of the ID key for a given entity type, or NULL if the entity
   *   type does not support fields.
   */
  protected function getEntityTypeIdKeyType(EntityTypeInterface $entity_type) {
    if (!$entity_type->entityClassImplements(FieldableEntityInterface::class)) {
      return NULL;
    }

    $field_storage_definitions = $this->getEntityFieldManager()->getFieldStorageDefinitions($entity_type->id());
    return $field_storage_definitions[$entity_type->getKey('id')]->getType();
  }

  protected function _getAddPageRoute($entity_type, $group_link_template) {
    if ($entity_type->getKey('bundle')) {
      $route = new Route($group_link_template);
      $route->setDefault('_controller', EntityController::class . '::addPage');
      $route->setDefault('_title_callback', EntityController::class . '::addTitle');
      $route->setDefault('entity_type_id', $entity_type->id());
      $route->setRequirement('_entity_create_any_access', $entity_type->id());
      return $route;
    }
  }

  /**
   * @param \Drupal\Core\Entity\EntityTypeInterface $entity_type
   * @param $group_link_template
   *
   * @return \Symfony\Component\Routing\Route
   */
  protected function _getCollectionRoute(EntityTypeInterface $entity_type, $group_link_template) {
    if ($entity_type->hasListBuilderClass() && ($admin_permission = $entity_type->getAdminPermission())) {
      /** @var \Drupal\Core\StringTranslation\TranslatableMarkup $label */
      $label = $entity_type->getCollectionLabel();

      $route = new Route($group_link_template);
      $route
        ->addDefaults([
          '_entity_list' => $entity_type->id(),
          '_title' => $label->getUntranslatedString(),
          '_title_arguments' => $label->getArguments(),
          '_title_context' => $label->getOption('context'),
        ])
        ->setRequirement('_permission', $admin_permission);

      return $route;
    }
  }

}